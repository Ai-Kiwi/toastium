...
void dtb_set_dst(u8 *new_dtb) { dtb = new_dtb; }
...
void dtb_dump(u8 *output_location, device_info_dump_response *response) {
    ...
    for (u32 byte_location = header.struct_offset;
         byte_location < header.struct_offset + header.struct_size;
         byte_location = byte_location + 4) {
        ...
        case 0x00000001: // node start
            ...
            if (node_depth > 0 &&
                node_stack[node_depth - 1]->first_child == NULL) {
                node_stack[node_depth - 1]->first_child = node;
            }
            node_stack[node_depth] = node;
            node_depth += 1;
            if (node_depth >= 14) {
                PANIC("DTB_STACK_DEPTH_TO_HIGH", byte_location, 0, 0);
            }
            s32 offset = 0;
...
u64 dtb_hart_cnt() { return 1; }

device_info *device_tree_prefix_get_child(
    const device_info *device, char *prefix,
    u32 iter_num) { // not actually cheapest way this could be stored really
