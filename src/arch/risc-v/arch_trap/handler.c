...
arch_trap_state arch_processes_trap_info[max_process_count];

void arch_trap_handler(arch_trap_state *trap_data) {//will have pointer input here that points to reg data on stack
    kernel_safety_test();
    ...
    kernel_trap_response trap_response = kernel_handle_trap(&trap, trap_data);
