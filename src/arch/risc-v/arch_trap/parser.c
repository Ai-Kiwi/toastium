...
void trapframe_parse(trap_data *trap) { // will have ptr input here that points
                                        // to reg data on stack
    trapframe *trap_frame_data = (trapframe *)TRAPFRAME_ADDRESS;

    const u64 is_interrupt = trap_frame_data->scause & BIT(63); // last bit
    const u64 trap_code = (trap_frame_data->scause) &
                          0x7FFFFFFFFFFFFFFF; // everything but last bit
