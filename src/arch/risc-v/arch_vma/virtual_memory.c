...
void arch_vma_assign(u64 page_table, u64 addr_space_id, u64 virt_addr, u64 phys_addr, u64 arg_flags) {
    u64 ppn0_offset; //least significant
    u64 ppn1_offset;
    u64 ppn2_offset; //most significant

    if (virt_addr % 4096) {
        PANIC("VMA_ASSIGN_NOT_PAGE_ALIGN", (s64)page_table, (s64)virt_addr, (s64)phys_addr);
    }
    ...
    if (ranged_virt_addr > 0x4000000000) {
        if (ranged_virt_addr < 0xffffffc000000000) {
            PANIC("VMA_ASSIGN_BETWEEN_USERSPACE_AND_KERNELSPACE", (s64)page_table, (s64)virt_addr, (s64)phys_addr)
        }
    ...
    asm volatile ("sfence.vma %0, %1" :: "r"(virt_addr), "r"(addr_space_id) : "memory"); //push cache update, only to current asid
}

void arch_vma_assign_kernel(u64 page_table, u64 addr_space_id, u64 virt_addr, u64 phys_addr, u64 arg_flags) {
    arch_vma_assign(page_table, addr_space_id, virt_addr, phys_addr, arg_flags);
}

void arch_vma_assign_user(u64 page_table, u64 addr_space_id, u64 virt_addr, u64 phys_addr, u64 arg_flags) {
    arch_vma_assign(page_table, addr_space_id, virt_addr, phys_addr, arg_flags | VMA_USER);
}

void arch_change_vma(kernel_process process) {

    u64 satp_value = 0
    | (((u64)process.asid) << 44) //ASID ID
