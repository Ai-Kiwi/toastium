...
static inline u64 create_leaf(u64 phys_addr, u64 arg_flags) {
    ...
    return ((((phys_addr - KERNEL_VMA_START) / 4096) << 10) | arg_flags |
            VMA_VALID);
}
...
static u64 expand_leaf(u64 *leaf, u64 branch_jmp_size) {
    ...
    *leaf = create_branch((u64)new_branch);
    ...
    for (u64 i = 0; i < 512; i++) {
        new_branch[i] = create_leaf(
            physical_location + (i * (branch_jmp_size / 512)), flags);
    }
...
static void shrink_branch(u64 *branch_loc, u64 branch_jmp_size) {
    ...
    if (branch_leafs[0] == 0) { // unmapped
        ...
        for (u64 i = 0; i < 512; i++) {
            ...
            can_shrink =
                can_shrink &&
                (iter_phys_loc == start_phys_loc + (leaf_jmp_size * i)) &&
                (iter_flags == start_access_flags) &&
                ((iter_flags & VMA_DONT_COMPRESS) == 0);
        }
    }
...
static void vma_replace_section(u64 table_root, u64 virt_addr_start,
                                u64 virt_addr_size, u64 phys_addr,
                                u64 access_flags, u64 vma_addr_asid) {
    ...
    if (normal_virt_addr > 0x4000000000 - virt_addr_size &&
        is_kernelspace == FALSE) { // spilling past max userspace
        PANIC("VMA_MAPPING_PAST_MAX_USERSPACE", virt_addr_start, virt_addr_size,
              phys_addr);
    }
    if (normal_virt_addr > 0x8000000000 - virt_addr_size &&
        is_kernelspace == TRUE) { // spilling past max userspace
        PANIC("VMA_MAPPING_PAST_MAX_KERNELSPACE", virt_addr_start,
              virt_addr_size, phys_addr);
    }
    ...
    while (mapping_left > 0) {
        if (ROUND_MOD_DOWN(cur_virt_addr, ppn2_jmp_size) == cur_virt_addr &&
        ...
        if (ROUND_MOD_DOWN(cur_virt_addr, ppn2_jmp_size) == cur_virt_addr &&
        ...
        } else if (ROUND_MOD_DOWN(cur_virt_addr, ppn1_jmp_size) ==
                       cur_virt_addr &&
        ...
        } else if (ROUND_MOD_DOWN(cur_virt_addr, ppn0_jmp_size) ==
                       cur_virt_addr &&
            ...
            PANIC("BAD_SIZE_LEFT_MAPPING", virt_addr_start, virt_addr_size,
                  phys_addr);
        }
    }
    ...
    if (((u64)pre_ppn1_table == 0x0) &&
        (pre_ppn1_cnt > 0 || pre_ppn0_cnt > 0)) {
        pre_ppn1_table =
            (u64 *)expand_leaf(&ppn2_table[cur_ppn2_offset], ppn2_jmp_size);
    }
    u64 *pre_ppn0_table =
        (u64 *)get_branch_loc(pre_ppn1_table[cur_ppn1_offset]);
    if (((u64)pre_ppn0_table == 0x0) && (pre_ppn0_cnt > 0)) {
        pre_ppn0_table =
            (u64 *)expand_leaf(&pre_ppn1_table[cur_ppn1_offset], ppn1_jmp_size);
    }

    // pre ppn 0
    for (u64 i = 0; i < pre_ppn0_cnt; i++) {
        pre_ppn0_table[cur_ppn0_offset] =
            create_leaf(phys_addr + cur_offset, access_flags);
    ...
    for (u64 i = 0; i < pre_ppn1_cnt; i++) {
        if (get_branch_loc(pre_ppn1_table[cur_ppn1_offset]) != 0) {
            destroy_branch((u64 *)pre_ppn1_table[cur_ppn1_offset],
                           ppn1_jmp_size);
        }
        pre_ppn1_table[cur_ppn1_offset] =
            create_leaf(phys_addr + cur_offset, access_flags);
    ...
    if (((u64)post_ppn1_table == 0x0) &&
        (post_ppn1_cnt > 0 || post_ppn0_cnt > 0)) {
        post_ppn1_table =
            (u64 *)expand_leaf(&ppn2_table[cur_ppn2_offset], ppn2_jmp_size);
    }
    ...
    for (u64 i = 0; i < post_ppn1_cnt; i++) {
        if (get_branch_loc(post_ppn1_table[cur_ppn1_offset]) != 0) {
            destroy_branch((u64 *)post_ppn1_table[cur_ppn1_offset],
                           ppn1_jmp_size);
        }
        post_ppn1_table[cur_ppn1_offset] =
            create_leaf(phys_addr + cur_offset, access_flags);
        cur_offset += ppn1_jmp_size;
        cur_ppn1_offset++;
    }
    ...
    if (post_ppn0_cnt > 0) {
        u64 *post_ppn0_table =
            (u64 *)get_branch_loc(post_ppn1_table[cur_ppn1_offset]);
        if (((u64)post_ppn0_table == 0x0) && (post_ppn0_cnt > 0)) {
            post_ppn0_table = (u64 *)expand_leaf(
                &post_ppn1_table[cur_ppn1_offset], ppn1_jmp_size);
        }

        for (u64 i = 0; i < post_ppn0_cnt; i++) {
            post_ppn0_table[cur_ppn0_offset] =
                create_leaf(phys_addr + cur_offset, access_flags);
            cur_offset += ppn0_jmp_size;
            cur_ppn0_offset++;
        }
    ...
    shrink_branch(&ppn2_table[cur_ppn2_offset],
                  ppn2_jmp_size); // shrink afterwards as it has to remain
                                  // expanded for ppn 0 to use

    // loop over again but with asid updates now
    cur_offset = 0;

    for (u64 i = 0; i < pre_ppn0_cnt; i++) {
        asm volatile("sfence.vma %0, %1" ::"r"(virt_addr_start + cur_offset),
                     "r"(asid)
                     : "memory");
        cur_offset += ppn0_jmp_size;
    }
...
void vma_map_kernel(process *proc, u64 virt_addr, u64 size, u64 phys_addr,
                    u64 access_flags) {
    vma_replace_section((u64)proc->vma_table, virt_addr, size, phys_addr,
                        access_flags | VMA_VALID | VMA_DIRTY | VMA_ACCESSED,
                        proc->vma_addr_space_id);
}

void vma_map_user(process *proc, u64 virt_addr, u64 size, u64 phys_addr,
                  u64 access_flags) {
    vma_replace_section((u64)proc->vma_table, virt_addr, size, phys_addr,
                        access_flags | VMA_VALID | VMA_USER | VMA_DIRTY |
                            VMA_ACCESSED,
                        proc->vma_addr_space_id);
}

void vma_unmap(process *proc, u64 virt_addr, u64 size) {
    vma_replace_section((u64)proc->vma_table, virt_addr, size, 0, 0,
                        proc->vma_addr_space_id);
}
...
// to keep it simple would just look at the one level and if it is all the same
...
void vma_create(process *proc) {
    ...
    /// allow read/write/execute, mark as global for optimization, is valid and
    /// also mark as already dirty and accessed for performance.
    const u64 access_mask =
        VMA_VALID | VMA_READ | VMA_WRITE | VMA_EXEC; // missing dirt and global
...
void vma_swap(process *process) {
    ...
    u64 satp_value = 0 | (((u64)process->vma_addr_space_id) << 44) // ASID ID
    ...
    if (max_asid != 0) {
        if (new_vma) {
            asm volatile("sfence.vma zero, %0" ::"r"(process->vma_addr_space_id)
                         : "memory");
        }
    } else {
        asm volatile("sfence.vma zero, zero" ::: "memory");
...
void vma_enable_read_user() { asm volatile("csrs sstatus, %0" ::"r" BIT(18)); }

void vma_disable_read_user() { asm volatile("csrc sstatus, %0" ::"r" BIT(18)); }