...
void test_allocator() {
    ...
    for (u64 i = 0; i < (value_cnt / 2); i++) {
        test_print_step("(1/8) Creating data 1st half... ", i * 2, value_cnt,
                        2);
    ...
    // need to test removing random items and make sure they are added back the
    // same
    ...
    test_print_next();
    for (u64 i = 0; i < blank_assigns;
         i++) { // create blank must be done after as else the follow index and
                // data page will be size jump
        test_print_step("(2/8) Creating blank, testing distance... ", i,
                        blank_assigns, 1);

        u64 location = mem_alloc(32);
        u64 location_change = location - old_location;
        if (location_change >
            9000) { // little over 8kb. Means that it has room for idx and page
                    // data change, also supports extra from start headers.
            PANIC("ALLOCATOR_CHANGE_LARGER_THEN_PAGE", location_change, i, 0)
        }
        old_location = location;
    }
    ...
    for (u64 i = 0; i < different_sizes; i++) {
        test_print_step("(3/8) Creating blank, all sizes... ", i,
                        different_sizes, 1);
    ...
    test_print_next();
    for (u64 i = 0; i < (blank_assigns / 10);
         i++) { // create blank must be done after as else the follow index and
                // data page will be size jump
    ...
    for (u64 i = (value_cnt / 2); i < (value_cnt); i++) {
        test_print_step("(5/8) Creating data 2nd half... ",
                        (i - (value_cnt / 2)) * 2, value_cnt, 2);
    ...
    // randomly sets values and frees them. On next assign these same values
    // should be set. also worth noting the high value then low value. This is
    // designed to test the cache system
