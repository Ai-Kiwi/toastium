...
static process *new_blank_process() {
    ...
    while (TRUE) {
        current_process =
            (process *)hashmap_get(&state.process_hashmap, state.process_upto);
        if (!current_process) {
            break;
        }
        state.process_upto++;
    ...
    *temp_process = new_process;
    ...
    u64 old_child = (u64)hashmap_insert(&state.process_hashmap,
                                        state.process_upto, (u64)temp_process);
    if (old_child) {
        PANIC("CREATE_BLANK_PROCESS_CONFLICTING_CHILD_PRESENT", old_child, 0,
              0);
    }
...
void processes_init(u64 hart_count) {
    ...
    for (u64 hart_id = 0; hart_id < hart_count; hart_id++) {
        ...
        if (hart_id != idle_process->process_id) {
            PANIC("IDLE_PROCESS_INVALID_ID", idle_process->process_id, hart_id,
                  0);
        }
        ...
        vma_map_user(idle_process, 0x1000, KERNEL_PAGE_SIZE,
                     (u64)&_kernel_idle_process, VMA_EXEC);
    }
}
...
void process_cleanup(process *proc) {
    ...
    if (highest_idx != proc->list_idx) {
        process *replacement_process =
            (process *)list_get(&state.running_list, proc->list_idx);
        if (replacement_process == 0x0) {
            PANIC("REPLACEMENT_PROCESS_REMOVE_SWAP_INVALID", proc->process_id,
                  highest_idx, proc->list_idx);
        }
...
void create_init_process() {
    ...
    vma_map_user(init_process, 0x1000, KERNEL_PAGE_SIZE,
                 (u64)&_kernel_init_process, VMA_EXEC | VMA_READ);
