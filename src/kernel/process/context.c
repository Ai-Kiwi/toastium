...
static void change_context(process *process) { vma_swap(process); }

// handles old process cleanup as well
void context_change_process(process *proc, u64 hart_id) {
    process *old_proc =
        (process *)((trapframe *)TRAPFRAME_ADDRESS)->process_ptr;
    ...
    case PROC_TYPE_DEAD:
        ...
        break;
    default:
        PANIC("UNHANDLED_PROCESS_TYPE", proc->process_type, proc->process_id,
              0);
        break;
    }
}
...
void context_bootstrap(u64 hart_id) {
    ...
    if ((u64)idle_process == 0) {
        PANIC("INVALID_IDLE_PROCESS_FOR_HART", (s64)hart_id, (s64)idle_process,
              0);
    }
