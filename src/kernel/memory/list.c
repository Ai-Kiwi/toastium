...
void list_remove(list *new_list, u64 item_idx) {//does swap remove so must move old one
    if (item_idx >= new_list->item_cnt) {
        PANIC("ATTEMPT_REMOVE_LIST_ITEM_OUT_BOUNDS", 0, 0, 0);
    }
...
void list_iter_create(list *new_list, list_iter *iter) {
    ...
    iter->max_iter_per_section = ((new_list->section_size - 8) / new_list->item_size);
}

u64 list_iter_next(list_iter *iter) {
    u64 cur_section_idx = iter->cur_upto % iter->max_iter_per_section;

    u64 ret = ((u64)iter->cur_list->first_section) + 8 + (iter->cur_list->item_size * cur_section_idx);
