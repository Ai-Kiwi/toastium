...
static void find_memory_regions(const device_info *device,
                                memory_region_list *memory_regions) {
    u32 mem_iter = 0;
    while (TRUE) {
        device_info *memory =
            device_tree_prefix_get_child(device, "memory", mem_iter);
        if (memory == NULL) {
            break;
        }

        device_info *memory_reg =
            device_tree_prefix_get_child(memory, "reg", 0);
        if (memory_reg == NULL) {
            break;
        }
        ...
        memory_regions->free_regions[memory_regions->free_cnt].start =
            big_endian_u64_to_host(value[0]) + KERNEL_VMA_START;
        memory_regions->free_regions[memory_regions->free_cnt].size =
            big_endian_u64_to_host(value[1]);
        memory_regions->free_cnt++;

        mem_iter += 1;
    }

    device_info *root_reserved_memory =
        device_tree_prefix_get_child(device, "reserved-memory", 0);
    if (root_reserved_memory == NULL) {
        PANIC("NO_RESERVED_MEMORY", 0x0, 0x0, 0x0)
    }

    mem_iter = 0;
    while (TRUE) {
        device_info *reserved_memory =
            device_tree_prefix_get_child(root_reserved_memory, "", mem_iter);
        if (reserved_memory == NULL) {
            break;
        }

        device_info *memory_reg =
            device_tree_prefix_get_child(reserved_memory, "reg", 0);
        if (memory_reg == NULL) {
            break;
        }
        ...
        memory_regions->reserved_regions[memory_regions->reserved_cnt].start =
            big_endian_u64_to_host(value[0]) + KERNEL_VMA_START;
        memory_regions->reserved_regions[memory_regions->reserved_cnt].size =
            big_endian_u64_to_host(value[1]);
...
static void remove_reserved_memory_regions(memory_region_list *memory_regions) {
    for (s32 i = 0; i < memory_regions->reserved_cnt; i++) {
        ...
        u64 reserved_region_end =
            reserved_region->start + reserved_region->size;
        ...
        for (s32 j = 0; j < memory_regions->free_cnt; j++) {
            ...
            // test overlap cases
            if (reserved_region->start <= free_region->start &&
                reserved_region_end >=
                    free_region_end) { // complete overlap/same size
                free_region->start =
                    memory_regions->free_regions[memory_regions->free_cnt - 1]
                        .start;
                free_region->size =
                    memory_regions->free_regions[memory_regions->free_cnt - 1]
                        .size;
            ...
            } else if (reserved_region->start <= free_region->start &&
                       reserved_region_end <
                           free_region_end) { // touching start
                free_region->start = reserved_region_end;
                free_region->size -= reserved_region_end - free_region->start;

            } else if (reserved_region->start > free_region->start &&
                       reserved_region_end >= free_region_end) { // touching end
                free_region->size -=
                    free_region->start - reserved_region->start;

            } else if (reserved_region->start > free_region->start &&
                       reserved_region_end < free_region_end) { // inside
                free_region->size -= free_region_end - reserved_region->start;

                memory_regions->free_regions[memory_regions->free_cnt].start =
                    reserved_region_end;
                memory_regions->free_regions[memory_regions->free_cnt].size =
                    free_region_end - reserved_region_end;
                memory_regions->free_cnt++;
                j--;
            }
        }
...
void pager_init() {
    ...
    u64 bitmap_location =
        (((s64)start_location + 7) / 8) * 8; // round to byte boundary
    ...
    u64 end = (s64)start_location - (u64)_kernel_start + 8 +
              (MAX_MEMORY_REGIONS * 24); // reserve for page idx locations data
    memory_locations.reserved_regions[memory_locations.reserved_cnt].start =
        start;
    ...
    // This is expensive for this however realistically ran once and small
    // dataset means small cost.
    ...
    if (memory_locations.free_cnt == 0) {
        PANIC("NO_MEMORY_REGIONS_FOR_PAGER", memory_locations.free_cnt,
              memory_locations.reserved_cnt, 0);
    }
    ...
    for (s32 i = 0; i < memory_locations.free_cnt; i++) {
        memory_region *region = &memory_locations.free_regions[i];
        u64 page_region_start = ((region->start / KERNEL_PAGE_SIZE) + 1) *
                                KERNEL_PAGE_SIZE; // round towards page chunks
...
u64 pg_alloc() { // will add cnt later u64 byte_cnt
    ...
    for (s32 i = last_free_bitmap; i < page_list_cnt; i++) {
        ...
        for (s32 j = page_start; j < page_cnt / 64;
             j++) { // TODO: actually does skip some but messy to use, will
                    // revisit
...
void pg_free(u64 location) {
    ...
    if (closest_bitmap == 0) {
        PANIC("PAGE_REMOVE_NO_OVERRIDE", (u64)page_location, closest_page_cnt,
              (u64)closest_page_list);
    }

    if (page_num < 0 || page_num >= closest_page_cnt) {
        PANIC("ATTEMPT_REMOVE_PAGE_OUT_OF_RANGE", page_num, closest_page_cnt,
              (u64)closest_page_list);
    }

    if (~closest_bitmap[bitmap_num] & BIT(bit_num)) {
        PANIC("DOUBLE_FREE_PAGE", (s64)page_location, (s64)page_num,
              (s64)closest_bitmap);
    }

    closest_bitmap[bitmap_num] &= ~BIT(bit_num);

    if (last_free_bitmap > closet_page_num ||
